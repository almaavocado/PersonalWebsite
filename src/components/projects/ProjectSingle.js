...
const ProjectSingle = ({ title, category, image, description, link, websiteLink }) => {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      ...
      <div className="rounded-xl shadow-lg hover:shadow-xl cursor-pointer h-full bg-secondary-light dark:bg-ternary-dark flex flex-col">
        ...
        <div className="flex flex-col justify-between px-4 py-6 flex-grow">
          ...
          <div className="flex justify-center">
            <button
              className="font-general-medium flex items-center text-md border border-green-600 py-2.5 sm:py-3 shadow-lg rounded-lg bg-white-800 hover:bg-green-900 text-gray-800 py-2 px-4 inline-flex items-center focus:ring-1 focus:ring-green-900 hover:text-white duration-500"
            ...
            {websiteLink && (
              <button
                ...
                onClick={() => window.location.href = websiteLink}
              >
                <FiLink className="mr-2 sm:mr-3 h-5 w-5 sm:h-6 duration-100" />
